...
def brighten(image, factor):
    ...
    # now we avoid mutating the orignal image

    new_im = Image(x_pixels=x_pixels, y_pixels=y_pixels,
                   num_channels=num_channels)

    # non-vectorized?? way by literally iterating all x,y and num_channels
    ...
    # using power of numpy instead of looping
...
def blur(image, kernel_size):
    ...
    # kernel_size // 2 represents how many neighbors to one side we need to look at
    neighbor_range = kernel_size // 2

    for x in range(x_pixels):
        for y in range(y_pixels):
            for c in range(num_channels):
                # this implementation takes the naive approach for the sake of easy understanding
                # more efficient appraach is to memoize to decrease the amount of calculations
...
if __name__ == '__main__':
    ...
    # lighten lake.png
    # brigntened_im = brighten(lake, 3)
    # brigntened_im.write_image('brightened.png')

    # darkened_im = brighten(lake, 0.2)
    ...
    # descr_contrast = adjust_contrast(lake, 0.3, 0.2)
    # descr_contrast.write_image('decreased_contrast.png')

    # blur_3 = blur(city,3)
    # blur_3.write_image('blur_3_city.png')
